...
class UserCommentModel {
  ...
  static getListWithLimitOffsetByField2(
    value1,
    value2,
    limit,
    offset,
    ...
    const query =
      "SELECT * FROM comment WHERE book_id = ? AND user_id = ? LIMIT ? OFFSET ?";
    const countQuery =
      "SELECT COUNT(*) as totalCount FROM comment WHERE book_id = ? AND user_id = ?";
    db.query(query, [value1, value2, limit, offset], (err, rows) => {
      if (err) {
        return callback({
          data: [],
          message: "Không thể lấy danh sách bình luận của người dùng",
      ...
      db.query(countQuery, [value1, value2], (err, countResult) => {
        if (err) {
          return callback({
            data: [],
            message: "Không thể đếm số lượng bình luận của người dùng",
